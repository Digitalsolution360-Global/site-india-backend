...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const results = await Promise.all([
            ...tables.map((t) => db.query(`SELECT COUNT(*) as count FROM \`${t}\``)),
            db.query(
        ...
        const counts = {};
        tables.forEach((t, i) => {
            counts[t] = results[i][0].count;
        });
        const citiesByCategory = results[n];
        const citiesByState = results[n + 1];
        const contactsOverTime = results[n + 2];
        const careersOverTime = results[n + 3];
        const recentContacts = results[n + 4];
        const recentCareers = results[n + 5];
        const postsByCategory = results[n + 6];
        const metroCitiesByCategory = results[n + 7];
...
router.post('/bulk-upload/:table', bulkUpload.single('file'), async (req, res) => {
    ...
    try {
        ...
        if (!dryRun && summary.inserted + summary.updated > 0) {
...
router.get('/:table', async (req, res) => {
    ...
    try {
        ...
        const rows = await db.query(
            `SELECT ${columns} FROM \`${req.params.table}\` ${whereClause} ORDER BY \`${sortBy}\` ${sortOrder} LIMIT ${lim} OFFSET ${off}`,
            params
        );
